...
from VisaRobot.mongo import db
from VisaRobot import MONGO_DB_URI, OWNER_ID
...
async def spammers(event):
    ...
    if (
        ...
        if senderr.username is None:
            ...
            hh = senderr.id
            final = f"[{st}](tg://user?id={hh}) you are detected as a spammer according to my algorithms.\nYou will be restricted from using any bot commands for 24 hours ! For Support join @KoraSupport"
        else:
            st = senderr.username
